...
def getWebsitesXy(test_size=0.2):
    ...
    dataset = pd.read_csv('/home/arthur/Uni/lab5/data/dataset.csv',
                          date_format='%d/%m/%Y %H:%M', parse_dates=True).drop(['CHARSET',
                                                              'URL',
                                                              'SERVER',
                                                              'WHOIS_COUNTRY',
                                                              'WHOIS_STATEPRO'],
                                                             axis=1).dropna()
    dataset['WHOIS_REGDATE']=pd.to_datetime(dataset['WHOIS_REGDATE'], format='mixed').apply(pd.Timestamp.toordinal) #.apply(datetime.toordinal)
    dataset['WHOIS_UPDATED_DATE']=pd.to_datetime(dataset['WHOIS_REGDATE'], format='mixed').apply(pd.Timestamp.toordinal) #.apply(datetime.toordinal)
