...
if __name__ == '__main__':
    lrm_cal_intercept = LRM(X_train, y_train, description='linear regression model with intercept for California housing prices', fit_intercept=True)
    lrm_cal_no_intercept = LRM(X_train, y_train, description='linear regression model with no intercept for California housing prices', fit_intercept=False)
    ...
    print("Prediction accuracy (R^2) for", lrm_cal_intercept, 'is', lrm_cal_intercept.get_score(X_train, y_train))
    print("Prediction accuracy (R^2) for", lrm_cal_no_intercept, 'is', lrm_cal_no_intercept.get_score(X_train, y_train))
    print("Prediction accuracy (R^2) of y_test for", lrm_cal_intercept, 'is', r2_score(y_test, y_pred_lrm_cal_intercept))
    print("Prediction accuracy (R^2) of y_test for", lrm_cal_no_intercept, 'is', r2_score(y_test, y_pred_lrm_cal_no_intercept))
